...
# D   500
# M   1000
# The value of a roman numeral are the digits added together. For example the roman numeral 'XX' is V + V = 10 + 10 = 20. 
# Typically the digits are listed from largest to smallest, so X should always come before I. Thus the largest possible 
# digits should be used first before the smaller digits (so to represent 50, instead of XXXXX, we should use L).
...
## I think this was just about if you can code given the constraints .... TC is Roughly nlogm

from bisect import bisect_left

def integer_to_roman(num):
  # Fill this in.
  mapper = [ (1, 'I'), (4, 'IV'), (5, 'V'), (9, 'IX'), (10, 'X'), (40, 'XL'), (50, 'L'), (90, 'XC'), (100, 'C'), (400, 'CD'), (500, 'D'), (900, 'CM'), (1000, 'M') ]
  result = []
  val = num 
  keys = [r[0] for r in mapper]
  while (val != 0):
    i = bisect_left(keys, val)
    if (i == len(mapper)) or mapper[i][0] != val: ## Bug - Did not check edge cases - for a number greater that 1000, i is out of range
      i = i - 1
    result.append(mapper[i][1])
    val = val - mapper[i][0]
...
if __name__ == "__main__":
    for i in range(1, 150):
      print("Decimal " + str(i) + " Roman " + str(integer_to_roman(i)))
